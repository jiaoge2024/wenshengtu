...
return "生图通道";
  ...
  function syncGenerationControls() {
    ...
    [
      ...
      elements.styleTemplate,
      elements.imageCount,
      elements.promptInput,
      elements.batchPromptInput,
      elements.clearBatchPrompts,
    ...
    if (elements.randomGenerateButton) {
      elements.randomGenerateButton.disabled = isGenerating;
    }
    if (elements.optimizePromptButton) {
      elements.optimizePromptButton.disabled = isGenerating || state.optimizing;
    }
    if (elements.generateButton) {
    ...
    if (elements.generateButton) {
      // 生成中显示「生成中…」等动态文案，否则恢复正式标签
      if (isGenerating) {
        var runningLabel = state.taskMode === "batch" ? "批量生成中…" : "生成中…";
        elements.generateButton.textContent = runningLabel;
      } else {
        elements.generateButton.textContent = getGenerateButtonLabel();
      }
    }
  ...
  function startRandomGeneration() {
    ...
    setStatus("随机生图", "已生成提示词：" + preset.title + "，可继续随机或点击开始创作");
  }
  ...
  async function optimizePrompt() {
    if (state.generating || state.optimizing) {
      return;
    }
